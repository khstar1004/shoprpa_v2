...
$reportRoot = Join-Path $repoRoot "build\browser-extension-content-jsdom-tests"
$reportPath = Join-Path $reportRoot "browser-extension-content-jsdom-tests-report.json"
if (-not (Test-Path -LiteralPath $reportRoot -PathType Container)) {
  New-Item -ItemType Directory -Path $reportRoot | Out-Null
}

$commandText = "corepack pnpm --dir frontend --filter @rpa/extension run test:content:jsdom"
$stdoutPath = Join-Path $reportRoot "browser-extension-content-jsdom-tests.stdout.log"
$stderrPath = Join-Path $reportRoot "browser-extension-content-jsdom-tests.stderr.log"
try {
  $result = Invoke-CapturedWithTimeout -FilePath "corepack" -Arguments @("pnpm", "--dir", "frontend", "--filter", "@rpa/extension", "run", "test:content:jsdom") -WorkingDirectory $repoRoot -TimeoutSeconds $TimeoutSeconds -StdoutPath $stdoutPath -StderrPath $stderrPath
...
if ($report.ok) {
  Write-Host "Browser extension content jsdom tests passed: $reportPath"
  Write-Host $report.summary
}
else {
  [Console]::Error.WriteLine("Browser extension content jsdom tests failed: $reportPath")
