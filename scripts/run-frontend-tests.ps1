...
$reportRoot = Join-Path $repoRoot "build\frontend-tests"
$reportPath = Join-Path $reportRoot "frontend-tests-report.json"
if (-not (Test-Path -LiteralPath $reportRoot -PathType Container)) {
  New-Item -ItemType Directory -Path $reportRoot | Out-Null
}

$commandText = "corepack pnpm --dir frontend run test:run"
$stdoutPath = Join-Path $reportRoot "frontend-tests.stdout.log"
$stderrPath = Join-Path $reportRoot "frontend-tests.stderr.log"
try {
  $result = Invoke-CapturedWithTimeout -FilePath "corepack" -Arguments @("pnpm", "--dir", "frontend", "run", "test:run") -WorkingDirectory $repoRoot -TimeoutSeconds $TimeoutSeconds -StdoutPath $stdoutPath -StderrPath $stderrPath
...
if ($report.ok) {
  Write-Host "Frontend unit tests passed: $reportPath"
  Write-Host $report.summary
}
else {
  [Console]::Error.WriteLine("Frontend unit tests failed: $reportPath")
