    ...
    def __validate__(cls, name: str, value):
        ...
        try:
            ...
            raise ParamException(
                PARAM_VERIFY_ERROR_FORMAT.format(name, value), "{}매개변수검증인증실패{}: {}".format(name, value, e)
            ) from e
...
class Bool:
    ...
    def __init__(self, value: bool):
        if not isinstance(value, bool):
            raise BaseException(TYPE_KIND_ERROR_FORMAT.format(value), "유형오류: {}".format(value))
        self._value = value
...
class Date:
    ...
    def __validate__(cls, name: str, value):
        if isinstance(value, Date):
            ...
            try:
                ...
                raise ParamException(
                    PARAM_VERIFY_ERROR_FORMAT.format(name, value), "{}매개변수검증인증실패{}".format(name, value)
                )

        raise ParamException(PARAM_VERIFY_ERROR_FORMAT.format(name, value), "{}매개변수검증인증실패{}".format(name, value))
    ...
    def __str__(self):
        if self.format in [TimeFormatType.YMD_CN, TimeFormatType.YMD_CN_HM, TimeFormatType.YMD_CN_HMS]:
            time_format = self.format.value.replace("년", "Nian").replace("월", "Yue").replace("일", "Ri")
            datetime_current = self.time.strftime(time_format)
            return datetime_current.replace("Nian", "년").replace("Yue", "월").replace("Ri", "일")
        else:
...
class WinPick(Pick):
    def __init__(self, *args, **kwargs):
        ...
        self.locator = None  # 결과가패키지locator직선연결사용locator
...
class Ciphertext:
    def __init__(self, s: str):
        self.str = s
        self.__key__ = None  # 복호화키, 결과가비어 있음설명아니요필요복호화, 비밀문서예복호화,암호화유형aes, 사용__방법법
