...
def web_default_strategy(service: "ServiceContext", strategy_svc: StrategySvc, cache=None) -> Optional[IElement]:
    ...
    if cache:
        ...
        bound = strategy_svc.start_control.BoundingRectangle  # 로선택 가능지정제어
    ...
    if (
        ...
        if smart_component_action == SmartComponentAction.START:
            ...
            raise Exception(f"선택연결매개변수 예외,찾을 수 없습니다SmartComponentAction {smart_component_action}")
    return ele