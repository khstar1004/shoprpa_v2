...
class HttpStorage(IStorage):
    ...
    def __http__(self, shot_url: str, params: Optional[dict], data: Optional[dict], meta: str = "post") -> Any:
        """post 요청 """
        logger.debug("요청 시작 {}:{}:{}".format(shot_url, params, data))
        ...
        logger.debug("요청 결과 {}:{}".format(shot_url, response.status_code))

        try:
            json_data = response.json()
            if json_data.get("code") != "000000":
                raise BaseException(
                    SERVER_ERROR_FORMAT.format(json_data.get("message", "")), "서버 오류{}".format(json_data)
                )
    ...
    def process_detail(self, project_id: str, mode: str, version: str, process_id: str) -> list:
        ...
        try:
            ...
            raise BaseException(PROCESS_ACCESS_ERROR_FORMAT.format(process_id), "데이터예외 {}".format(e))
