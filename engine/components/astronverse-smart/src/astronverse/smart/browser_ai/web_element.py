...
class WebElement:
    ...
    def slider_hover(
        ...
        * @param progress_element, 요소(의)
        * @param percent_value, 분(0-100)
        * @param drag_direction, 방법("left", "right", "up", "down")
        * @param drag_type, 유형("start" 에서열기 , "current" 현재위치)
        * @param duration, 시간
        """
        # 변환문자열매개변수로
        direction_map = {
        ...
        BrowserElement.slider_hover(
            browser_obj=self.browser,
            element_slider=self.element_data,  # 현재요소로
            element_progress=progress_element.element_data,  # 입력의요소
    ...
    def execute_javascript(self, code: str, argument=None, execution_world="ISOLATED") -> str:
        ...
        * @return `str`, return the str result of the javascript execution
        ...
        result = BrowserScript.js_run(
            ...
            content=js_code,
            params=params,
            element_data=self.element_data,  # 현재원데이터
        )
