...
class Docx:
                ...
                if text_file_path and os.path.exists(text_file_path):
                    for encoding in ("utf-8", "gbk", "gb2312", "utf-16"):
                        ...
                        raise BaseException(
                            ...
                            "불가해제코드 txt 파일코드, 요청확인파일로 UTF-8 또는 GBK 코드!",
                        )
                else:
                    raise BaseException(
                        DOCUMENT_PATH_ERROR_FORMAT.format(text_file_path),
                        "txt 파일찾을 수 없습니다, 확인하세요경로!",
                    )
            if not font_color:
                font_color = "0,0,0"
            text_format = {
    ...
    def insert_sep(doc: DocumentObject, sep_type: InsertionType = InsertionType.PARAGRAPH):
        ...
        try:
            ...
            raise BaseException(DOCUMENT_READ_ERROR_FORMAT.format(doc), "삽입실패, 확인하세요문서여부열기!") from e
