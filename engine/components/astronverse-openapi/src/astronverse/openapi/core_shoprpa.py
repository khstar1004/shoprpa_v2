...
class OpenapiShoprpa:
    ...
    def template_ocr(header_dict: dict, files: list, type_code: str, url: str, key_name: str) -> list:
        ...
        for image in files:
            ...
            if ret.status_code != 200:
                raise BaseException(AI_SERVER_ERROR, "ai서비스서버없음또는오류 {}".format(ret))
            text = (
    ...
    def common_ocr(header_dict: dict, files: list) -> list:
        ...
        for image in files:
            ...
            content = OpenapiShoprpa.__analyse_ocr_result__(ret_dict)
    ...
    def __analyse_ocr_result__(data: dict) -> str:
        ...
        try:
            ...
            return ""
        # 관리셀행데이터, 출력거리기존의위치재그룹데이터
        content = ""
        y = 0
        first = True
        for line in lines:
            if line.get("words") and line.get("coord"):
                # 결과가line의y값소10, 이면로예일행데이터
