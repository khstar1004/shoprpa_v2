...
class VerifyCode:
    ...
    def picture_code(
        ...
        logger.info("인증 코드반환값: {}".format(code_result))
        if not code_result:
    ...
    def slider_code(
        ...
        logger.info("인증 코드반환값: {}".format(drag_distance))
        drag_distance = drag_distance + offset
        logger.info("추가입력량후의량: {}".format(drag_distance))
    ...
    def click_code(
        ...
        logger.info("인증 코드반환값: {}".format(click_result))
        if not click_result:
            raise BaseException(MSG_EMPTY_FORMAT, "")
