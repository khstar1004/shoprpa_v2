...
class PDF:
    ...
    def get_pdf_images(
        ...
        """
        가져오기PDF파일의이미지
        :param file_path: PDF파일 경로
        :param pwd: PDF파일비밀번호(결과가있음)
        :param select_range: 선택(예: "ALL"또는"PART")
        :param page_range: 페이지(예: "1-5"또는"1,3,5")
        :param save_dir: 이미지저장경로
        :param image_type: 이미지유형(예: "png"또는"jpeg")
        :param prefix: 이미지파일이름전
    ...
    def merge_pdf_files(
        ...
        """
        병합다중개PDF파일
        :param merge_type: 파일병합유형(폴더또는파일)
        :param file_folder_path: 폴더 경로
        :param files_path: 파일 경로
        :param save_dir: 저장경로
        :param new_file_name: 병합후의파일이름
        :param new_pwd_flag: 여부새비밀번호
        :param new_pwd: 새비밀번호
        :param exist_handle_type: 파일저장된 관리유형
        :return: 병합후의파일 경로
        """
