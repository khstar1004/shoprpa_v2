...
def linux_open_folder(folder_path: str = ""):
    ...
    subprocess.Popen(
        ...
        start_new_session=True,  # Linux 단말
        close_fds=True,  # 닫기없음사용 fd
    )
...
def list_to_excel(path_list: list = None, excel_path: str = ""):
    ...
    data = {
        "순서": range(1, len(path_list) + 1),
        "이름": [os.path.basename(path) for path in path_list],
        "경로": path_list,
    }
...
def get_files_in_folder(folder_path: str = "", general=False) -> list:
    ...
    Args:
        folder_path: 폴더 경로
        general: 여부패키지파일 False 패키지 True 아니오패키지 False
    """
    try:
        ...
        raise Exception(f"가져오기폴더아래파일목록실패, 폴더 경로: {folder_path}")
