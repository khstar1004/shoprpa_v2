...
class System:
    ...
    @atomicMg.atomic(
        ...
        inputList=[
            ...
            atomicMg.param("printer_type"),  # 인쇄  시스템및지정
    ...
    def printer(
        ...
        file_path: str = "",
        folder_path: str = "",
        printer_type: PrinterType = PrinterType.DEFAULT,  # 인쇄  시스템및지정
        printer_name: str = "",
        ...
        if batch_print == BatchType.SINGLE:
            if not file_is_exists(file_path):
                raise BaseException(
                    FILE_PATH_ERROR_FORMAT.format(file_path), "파일찾을 수 없습니다또는경로정보있음오류, 확인하세요경로정보"
                )
            print_file = file_path
        elif batch_print == BatchType.BATCH:
            if not folder_is_exists(folder_path):
                raise BaseException(
                    FOLDER_PATH_ERROR_FORMAT.format(folder_path), "폴더찾을 수 없습니다또는경로정보있음오류, 확인하세요경로정보"
                )
            print_file = []
