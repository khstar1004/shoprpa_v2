...
class Compress:
    ...
    def compress(
        ...
        if file_type == FileFolderType.FILE and not file_path:
            raise ValueError("대기압축파일 경로비워 둘 수 없습니다, 확인하세요선택내용")
        if file_type == FileFolderType.FOLDER and not folder_path:
            raise ValueError("대기압축폴더 경로비워 둘 수 없습니다, 확인하세요선택내용")
        if file_type == FileFolderType.BOTH and not (file_path or folder_path):
            raise ValueError("대기압축파일및폴더 경로비워 둘 수 없습니다, 확인하세요선택내용")
    ...
    def uncompress(
        ...
        if not os.path.isfile(source_path):
            raise BaseException(FILE_PATH_ERROR_FORMAT.format(source_path), "파일찾을 수 없습니다, 확인하세요파일 경로")
