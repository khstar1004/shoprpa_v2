...
const showDialog: UtilsManagerType['showDialog'] = async (dialogProps) => {
  ...
  const isDirectory = file_type === 'folder' // 열기파일
  const isMultiple = file_type === 'files' ? true : (file_type === 'file' ? multiple : false) // 단일선택
  ...
  const dialogObj: DialogObj = { title: '선택파일 디렉터리', defaultPath, properties, filters }
