...
export class IncrementalASTParser {
  ...
  public deleteNode(nodeId: string) {
    ...
    if (!node)
      return
    if (!node.parent)
      throw new Error('삭제할 수 없는 노드입니다.')
  ...
  public insertNodeAfter(targetId: string, insertIdx: number, newNodeData: ProcessNode[] | ASTNode[], isUINull: boolean, isAdd: boolean = true) {
    ...
    if (insertIdx !== 0 && this.nodeMap.size !== 0) {
      ...
      if (!targetNode)
        throw new Error(`대상 노드를 찾을 수 없습니다: ${targetId}`)
      parentNode = this.defineParent(targetNode, newNodeData[0].type)
      if (!parentNode)
        throw new Error(`대상 노드의 상위 노드를 찾을 수 없습니다: ${targetId}`)
  ...
  private safeInsert(params: {
    ...
    nodes.some((node) => {
      if (this.checkCyclic(parent, node)) {
        throw new Error('자기 자신을 하위 노드로 추가할 수 없습니다.')
      }
      return false
    })
    ...
    switch (node.type) {
      case IF_TEXT:
        if (!this.hasRelatedChild(node, IF_END_TEXT))
          node.raw.error = `${node.type}에 닫는 ${IF_END_TEXT}가 필요합니다.`
        break
      case ELSE_IF_TEXT:
      case ELSE_TEXT:
      case IF_END_TEXT:
      ...
      case WHILE_TEXT:
        if (!this.hasRelatedChild(node, FOR_END_TEXT))
          node.raw.error = `${node.type}에 닫는 ${FOR_END_TEXT}가 필요합니다.`
        break
      case FOR_END_TEXT:
      ...
      case GROUP_TEXT:
        if (!this.hasRelatedChild(node, GROUP_END_TEXT))
          node.raw.error = `${node.type}에 닫는 ${GROUP_END_TEXT}가 필요합니다.`
        break
      case GROUP_END_TEXT:
      ...
      case TRY_TEXT:
        if (!this.hasRelatedChild(node, TRY_END_TEXT))
          node.raw.error = `${node.type}에 닫는 ${TRY_END_TEXT}가 필요합니다.`
        break
      case CATCH_TEXT:
      case FINALLY_TEXT:
      case TRY_END_TEXT:
  ...
  private hasRelatedParent(node: ASTNode, types: string[]) {
    ...
    if (flag) {
      ...
      node.raw.error = `${node.type}에 연결된 ${text} 블록이 없습니다.`
    }
  }
