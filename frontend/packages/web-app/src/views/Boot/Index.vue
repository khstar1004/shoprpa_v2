...
utilsManager.listenEvent('scheduler-event', (eventMsg) => {
  let msgObject: any
  try {
    msgObject = JSON.parse(base64ToString(eventMsg))
  }
  catch (error) {
    console.error('scheduler-event 메시지 파싱 실패:', error)
    return
  }
...
function loginSuccess(_userInfo: any) {
...
onMounted(async () => {
  ...
  if (appConfig?.remote_addr) {
    try {
      ...
      console.error('remote_addr 설정이 올바르지 않습니다:', error)
    }
  }
})
...
window.onload = async () => {
  ...
  if (err) {
    console.error('main_window_onload 호출 실패: ', err)
  }
}
