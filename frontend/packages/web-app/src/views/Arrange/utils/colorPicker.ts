...
export function isRgb(str: string) {
  const regex = /^(\d{1,3}),\s?(\d{1,3}),\s?(\d{1,3})$/ // 매칭rgb형식의정상이면테이블방식
  const match = str.match(regex) // 사용match방법법행매칭
  if (match) {
    // 만약매칭성공
    const r = Number.parseInt(match[1]) // 가져오기 색상값
    const g = Number.parseInt(match[2]) // 가져오기 색상값
    const b = Number.parseInt(match[3]) // 가져오기 색상값
    if (r >= 0 && r <= 255 && g >= 0 && g <= 255 && b >= 0 && b <= 255) {
      // 중단RGB값여부에서합치기법내부
      return true // 만약합치기법, 반환true
    }
  }
  return false // 만약아니오합치기법, 반환false
}
