        ...
        .map((i, idx) => {
          ...
          if (i.logic === 'in' || i.logic === 'notIn') {
            ...
            if (i.sort && !i.childCondition) {
              res = `선택열로${i.sort}의값${specialItem?.label}${i.conditionalValue}`
            }
              ...
              const tips = i.childCondition.map((child) => {
                ...
                return `선택열로${i.sort}의값${specialItemExcel?.label}${condition.join(' ')}`
              })
              res = `${tips.join()}`
            }
            else {
...
export function validateConditional({ operands, operators }, formObj) {
  ...
  // operators==='and' 건파일전체가득, 이면이표시, operators==='or' 건파일일부가득,
