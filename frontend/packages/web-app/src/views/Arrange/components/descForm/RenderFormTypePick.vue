...
const renderInfo = computed(() => {
  ...
  return {
    operatorsOptions: menus, // 아래목록
    text: noEmpty ? desc : getDefaultText(itemType), // 텍스트
    noEmpty,
    img: noEmpty ? getPickImg(itemData, itemType) : '', // 이미지
  }
})
...
function editElement(params: { id: string, name: string }) {
  ...
  if (eleItem && eleItem.commonSubType === 'batch') { // 가져오기요소대상, 열기데이터가져오기선택
...
<template>
  ...
  <a-modal
    ...
    :title="`선택${PickTypeText[itemType]}`"
    :width="600"
    :z-index="10"
    @ok="handleOk"
    @cancel="handleCancel"
