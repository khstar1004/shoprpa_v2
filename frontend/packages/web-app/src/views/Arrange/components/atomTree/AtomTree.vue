...
const collapsed = ref(false) // 열기모든점
...
const fullTreeData = computed<AtomTreeNode[]>(() => {
  ...
  // 에서 list 중저장에서재복사의 key, 원인이필요에게매개점추가일개일의 id, 통신경과경로의 key 완료
...
watchDebounced([searchAtom, fullTreeData], ([searchKey, treeList], [oldSearchKey]) => {
  ...
  /**
   * 로완료가능에서검색시보관열기의점상태, 필요기록일아래전의열기 점상태,
   * 검색결과시, 를전의열기 점상태값에게 expandedKeys
   */
  // 열기 검색시
...
const onExpand: TreeProps['onExpand'] = (_, { expanded }) => {
  if (expanded) {
