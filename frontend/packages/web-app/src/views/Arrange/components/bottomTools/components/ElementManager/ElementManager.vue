...
const height = inject<Ref<number>>('logTableHeight', ref(180)) // 만약있음비고입력, 기본값값로320
...
function handleRePick(data) {
 if (data.commonSubType === 'batch') { // 가져오기요소대상, 열기데이터가져오기선택
...
<template>
 ...
 <div class="pick-img">
 <Image v-if="useElements.selectedElement.imageUrl" :title="$t('fullSizeImage')" :src="getImageURL(useElements.selectedElement.imageUrl)" alt="요소이미지" />
 </div>
 </div>
 </div>
 </div>
...
:deep(.ant-tree-switcher-icon) {
 svg {
 vertical-align: middle !important; // 점열기 아이콘수직직선중
 }
}
</style>
