...
const atomFormDisabled = inject<Ref<boolean>>('atomFormDisabled', ref(false)) // 지정컴포넌트금지입력
const cursorStore = useCursorStore()
const flowStore = useFlowStore()
const container = ref(generateInputVal(itemData))
const selectValue = ref(generateInputVal(itemData))
const pickLoading = ref(false)
const isEdit = computed(() => !itemData?.noInput) // 여부가능 noInput로true시할 수 없음
...
watch(() => itemData.value, () => { // 관리지정대화상자이미지다중개파일업데이트
...
function handleInput(event: Event, itemData: RPA.AtomDisplayItem) {
  // 저장현재 activeAtom.id, 에서 debounce 지연 activeAtom 가져오기 업데이트까지오류의기존가능
...
.editor {
  ...
  white-space: pre; // 보관행기호및빈격식, 아니오행
  overflow: auto;
