...
const currentNodeIndex = ref(0) // 현재점검색
const openAddNode = ref(false) // 여부열기추가점팝업
const addNodeFormRef = ref(null) // 추가점테이블단일사용
const addNodeForm = reactive({ name: '' })
const currentKey = ref('') // 현재선택중의점key
...
watch(() => props.nodeSource, () => {
 currentNodeIndex.value = 0 // 재현재점검색
 currentKey.value = Math.random().toString(36).substring(2, 15) // 업데이트현재key으로트리거attrTable다시 
}, { immediate: true })
</script>
