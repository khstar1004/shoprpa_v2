...
const props = withDefaults(defineProps<{
 items: T[] // 목록 데이터
 itemSize?: number // 목록높음정도, 높음정도필요전송입력
 minItemSize?: number // 목록소높음정도, 동작높음정도필요전송입력
 itemKey: PropertyKey // 일키
 buffer?: number // 완화수
 beforeAdd?: (e: any) => void | boolean // 반환 false 가져오기메시지삽입
}>(), {
 buffer: 20,
})
...
function scrollToIndex(index: number, duration: number = 300) {
 ...
 throw new Error(
 `[ScrollTo Error] 목록표시검색${index}(있음: 0~${realItems.value.length - 1})`,
 )
 }
