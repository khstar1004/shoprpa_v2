...
export default {
  ...
  methods: {
    createWs(url: string) {
      ...
      this.ws.bindMessage((res: string) => { // 관리ws메시지
        const { data, channel } = JSON.parse(res)

        this.getLogData(data)

        // 실행 결과, 실행출력오류, 실행기기오류대기예외출력시, 닫기socket그리고재상태
    ...
    getIconState(item, index) {
      ...
      indexStatus = this.logData.length === 1 ? 'current' : indexStatus // 일건시
