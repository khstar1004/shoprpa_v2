...
function handExecuteTimeChange() {
 ...
 break
 case 'days':
 // 에서매일의 formState.hours:formState.minutes 실행일회
 taskExecuteList.value = [
 ...
 break
 case 'weeks':
 // 에서매주의 formState.weeks:formState.hours:formState.minutes 실행일회, weeks 예개배열
 taskExecuteList.value = [
 getEveryWeeksExecuteTime(formState.weeks, formState.hours, formState.minutes, 0),
 ]
 break
 case 'months':
 // 에서매 months 의 weeks, formState.hours:formState.minutes 실행일회, months 예개정수, weeks 예개배열
 taskExecuteList.value = [
...
async function checkFirstTime() {
 ...
 message.warning('종료 시간이 첫 실행 시간보다 빠릅니다.')
 return false
 }
 else {
 return true
...
function viewCorn() {
 utilsManager.openInBrowser('https://www.shoprpa.com/help/article/850')
}
