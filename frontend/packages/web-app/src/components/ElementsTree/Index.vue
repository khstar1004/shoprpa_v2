...
// 통신경과 name 조회의 id(있음다중개이름, 반환일개매칭의)
...
function scrollInto(name: string) {
  ...
  if (!isExist)
    return // 찾을 수 없습니다, 이면아니오필요
  scrollToName(name)
}
// 까지treeData, 검색및전체예분열기의데이터
...
//  expandedKeys 변수, 저장까지 localStorage
...
<template>
  ...
  <a-empty v-if="searchVal && searchData.length === 0" description="검색 결과가 없습니다." />
</template>
