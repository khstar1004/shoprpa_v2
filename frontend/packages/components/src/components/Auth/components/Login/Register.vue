...
const headerTitle = computed(() => {
  if (edition === 'saas' && authType === 'uap') {
    return {
      title: currentMode.value === 'REGISTER' ? t('auth.registerShoprpa') : t('auth.consult'),
      actionText: currentMode.value === 'REGISTER' ? t('auth.consult') : t('auth.registerShoprpa'),
    }
  }
  if (edition === 'saas' && authType === 'casdoor') {
    return { title: t('auth.registerShoprpa'), actionText: '' }
  }
