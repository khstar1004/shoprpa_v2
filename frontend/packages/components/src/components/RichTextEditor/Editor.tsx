      ...
      return (
        <div class="p-0">
          <div class="h-10 border-b border-border flex items-center px-4 gap-2">
            ...
            <IconButton title="추가문서문자" active={editor.value?.isActive('bold')} onClick={() => editor.value?.chain().focus().toggleBold().run()}>
              <BoldOutlined />
            </IconButton>
            <IconButton title="있음순서목록" active={editor.value?.isActive('orderedList')} onClick={() => editor.value?.chain().focus().toggleOrderedList().run()}>
              <OrderedListOutlined />
            </IconButton>
            <IconButton title="없음순서목록" active={editor.value?.isActive('bulletList')} onClick={() => editor.value?.chain().focus().toggleBulletList().run()}>
