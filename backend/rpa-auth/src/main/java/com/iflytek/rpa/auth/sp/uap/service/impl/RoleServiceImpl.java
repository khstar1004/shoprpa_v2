...
public class RoleServiceImpl implements RoleService {
    ...
    public AppResponse<String> addRole(CreateRoleDto createRoleDto, HttpServletRequest request) {
        ...
        if (addResult.isFlag()) {
            ...
            return AppResponse.success(data != null ? data.toString() : "추가성공");
        }
        log.error("추가역할실패: {}", addResult.getMessage());
    ...
    public AppResponse<String> deleteRole(DeleteCommonDto deleteCommonDto, HttpServletRequest request) {
        ...
        // 서비스이면: 역할[지정되지 않았습니다]할 수 없음삭제
        if (roleId.equals("1")) {
            return AppResponse.error(ErrorCodeEnum.E_PARAM, "역할[지정되지 않았습니다]할 수 없음삭제");
        }
